...
const SaleCarrusel: React.FC<SaleCarruselProps> = ({ titulo, catalogo }) => {
    return (
        <section className="w-screen max-w-none left-1/2 right-1/2 -translate-x-1/2 py-10 bg-blue-900" style={{ position: 'relative', left: '50%', right: '50%', transform: 'translateX(-50%)' }}>
                ...
                {catalogo.map((oferta) => (
                    ...
                    >
                        ...
                        <div className="p-3 flex-1 flex flex-col">
                            ...
                            {oferta.price && <div className="text-sky-600 font-bold text-sm mb-1">{oferta.price}</div>}
                            {oferta.description && <div className="text-xs text-gray-500 line-clamp-2 flex-1">{oferta.description}</div>}
                        </div>
                    </a>
                ))}
            </div>
